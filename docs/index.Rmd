...
In order to improve the relevancy of search results, Discovery's Search team has decided to test a new document ranking method called [BM25](https://en.wikipedia.org/wiki/Okapi_BM25), which would replace the term frequency–inverse document frequency ([tf–idf](https://en.wikipedia.org/wiki/Tf%E2%80%93idf)) method that is currently used. We saw the following results from our analysis:
...
* __PaulScore__, a metric of search results' relevancy that relies on the position of the clicked result[s] (bigger is better); see [PaulScore Definition](#paulscore_definition) below for more details
