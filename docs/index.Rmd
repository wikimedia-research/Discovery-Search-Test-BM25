...
subtitle: "Revised on `r as.character(Sys.Date(), '%d %B %Y')`"
author:
...
date: "Originally published on `r as.character(as.Date('2016-10-11'), '%d %B %Y')`"
output:
  html_document:
    code_folding: hide
    css: style.css
...
_**Revision Notes**: In the original version of this report, some of the engagement and PaulScore calculations did not filter out searches with zero results. In this revised version, we added a filter that only keeps searches with some results (since that means users actually have something to click on). The clickthrough rates and PaulScores are slightly higher all around, so the overall conclusions are the same, but we still wanted this report to be as accurate as it can be._
